...
func run() -> void:
	...
	s._music_slider.slider.grab_focus()
	s._music_slider.set_percent(0.0)
	check(s.audio.is_muted(&"Music") and s._music_slider.slider.has_focus(), "Music slider reaches mute without losing focus")
	s._music_slider.set_percent(100.0)
	s._sfx_slider.set_percent(0.0)
	check(s.audio.is_muted(&"SFX") and not s.audio.is_muted(&"Music"), "SFX slider stays independent")
	s._sfx_slider.set_percent(100.0)
