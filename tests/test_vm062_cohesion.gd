...
func run() -> void:
	...
	check(s.audio.is_muted(&"Music"),"Credits music toggle")
	s._music_button.pressed.emit()
	...
	check(s._c2.buttons.size()==5 and s._c2.buttons[0].has_focus(),"Pause focus order begins with Resume")
	s._music_button.pressed.emit()
	...
	s._music_button.pressed.emit();s._sfx_button.pressed.emit()
