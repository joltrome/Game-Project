...
func _initialize() -> void:
	...
	var edits: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(args[0].path_join("sample-edits.json")))
	for candidate: String in edits.candidates:
		var frame_count := int(edits.candidates[candidate].frames)
